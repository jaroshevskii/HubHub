...
class TestViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 0 {
            // Standard section, handle the selection as needed
            handleStandardCellSelection(at: indexPath.row)
        } else {
            // Custom section, handle the selection as needed
            handleCustomCellSelection(at: indexPath.row)
        }
    }

    func handleStandardCellSelection(at index: Int) {
        // Implement your logic for standard cell selection
        print("Selected standard cell: \(standardOptions[index])")
    }

    func handleCustomCellSelection(at index: Int) {
        // Implement your logic for custom cell selection
        if index == 0 {
